...
var fileUpload = function (job, io) {
    ...
    this._cookies = global._cookies ? global._cookies['clipwatching.com'] : (global._cookies = {});

    this._iologin = "https://clipwatching.com/";
    this._server = "http://clipwatching.com/?op=upload";

    this.server = 'clipwatching.com';
    this.host = 'clipwatching.com';
...
fileUpload.prototype.initAuth = function () {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (_this._cookies) {
            var _xfsts = _this._cookies.match(/xfsts=(.*?)\;?(\s+)/i);
            _this._sess_id = _xfsts ? _xfsts[1] : null;
        ...
        var formData = {
            ...
            redirect: 'https://clipwatching.com/?op=upload_file'
        };
        ...
        request({
            ...
            _this._cookies = global._cookies['clipwatching.com'] = cookie_string;

            if (/xfsts=/ig.test(cookie_string)) {
                var _xfsts = cookie_string.match(/xfsts=(.*?)\;?(\s+)/i);
                _this._sess_id = _xfsts ? _xfsts[1] : null; 

                const $ = cheerio.load(body);

                _this.uploadServer = $('form[name="file"]').attr('action');

                return resolve();
            } else {
                _this._cookies = global._cookies['clipwatching.com'] = null;
...
fileUpload.prototype._getServer = function () {
    ...
    return new Promise(function (resolve, reject) {
        ...
        request(opts, function (err, response, body) {
            ...
            if (!_this.uploadServer || !_this.uploadServer.match(/^(http(s)?):\/\//ig)) {
                _this._cookies = global._cookies['clipwatching.com'] = null;
        ...
        var r = request(opts).on('error', function (error) {
            ...
            response.on('data', function (data) {
                ...
                if (!_id || _id == "") {
                    ...
                    _this.link = `https://clipwatching.com/${_id}/${_this.filenameRD}.html`;
                    return resolve();
                }
