...
var fileUpload = function (job, io) {
    ...
    this.uploadInfo = "https://api.vidcloud.co/v1/file/upload";

    this.server = 'vidcloud.co';
    this.host = 'vidcloud.co';
...
fileUpload.prototype._getServer = function () {
    ...
    return new Promise(function (resolve, reject) {
        ...
        _this.uploadInfo = `${_this.uploadInfo}?login=${_this._apilogin}&key=${_this._apikey}`;
...
fileUpload.prototype.initFile = function () {
    var _this = this;
    return new Promise(function (resolve, reject) {
        ...
        detect.fromFile(_this.filePath, function (err, result) {
            ...
            if (_this.allowedMime.indexOf(mime) == -1) {
                return reject(`File type not allowed ${mime}`);
            } else {
                return resolve();
            }
            ...
            response.on('data', function (data) {
                ...
                if (!data) {
                    ...
                    if (data && data['status'] != 200) {
                        error = data['file'] ? data['file'].toString() : "Internal error, please retry"
                        return reject(error);
                    } else if (error) {
                        return reject(error);
