...
var fileUpload = function (job, io) {
    ...
    this.uploadServer = 'https://ul.mixdrop.co/api';
    this._inkBuild = 'https://mixdrop.co/f/';

    this.server = 'mixdrop.co';
    this.host = 'mixdrop.co';
...
fileUpload.prototype.datainit = function () {
    ...
    return new Promise(function (resolve, reject) {
        Hosts.findOne({
            ...
            _this._apiemail = result.apiemail;
            _this._apikey = result.apikey;

            if (result.use_account == false || !_this._apiemail || !_this._apikey) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var r = request(opts).on('error', function (error) {
            ...
            response.on('data', function (data) {
                ...
                if (!data) return reject("Host reject Upload with \"Internal Server Error\", please retry");

                try {
                    data = JSON.parse(data);
                    var _success = data['success'] || false;

                    if (_success == false) return reject("Upload Failed");

                    var _id = data['result']['fileref'];

                    if (!_id || _id == "") {
                        return reject("Invalid Url type..");
                    } else {
                        _this.link = `${_this._inkBuild}${_id}`;
                        return resolve();
                    }
                } catch (e) {
                    var _merror = "Error parsing Received Data";
                    if (e && /Internal Server Error/i.test(data)) {
                        _merror = "Host reject Upload with \"Internal Server Error\", please retry";
                    }
                    return reject(_merror);
                }
        ...
        var form = r.form();
        form.append('email', _this._apiemail)
        form.append('key', _this._apikey)
